...
def save_and_push_model(
    model, tokenizer, repo_id: str, HF_TOKEN: str, enable_hf: bool = True
):
    """
...
dataset = load_dataset(
    ...
    data_files={
        ...
        ),  # Wildcard to load all train files
        "validation": os.path.join(
            dataset_path, "validation-*.parquet"
        ),  # Wildcard for validation files
...
train_dataset = [
    ...
    for sample in tqdm(train_dataset, desc="Processing Train Data", unit="sample")
]

print("\nProcessing validation dataset...")
val_dataset = [
    convert_to_conversation(sample)
    for sample in tqdm(val_dataset, desc="Processing Validation Data", unit="sample")
]
